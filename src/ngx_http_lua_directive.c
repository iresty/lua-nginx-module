...
ngx_http_lua_rewrite_by_lua(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (handlers->nelts >= NGX_HTTP_LUA_MAX_HANDLER_PER_PHASE) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "the number of rewrite_by_lua* directives "
                           "exceeds %d",
                           NGX_HTTP_LUA_MAX_HANDLER_PER_PHASE);
        return NGX_CONF_ERROR;
    }
...
ngx_http_lua_access_by_lua(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    ngx_http_lua_main_conf_t        *lmcf;
    ngx_http_lua_loc_conf_t         *llcf = conf;
    ngx_array_t                     *handlers = llcf->access_handlers;
    ngx_http_lua_phase_handler_t    *ph;
    ...
    if (handlers == NULL) {
        ...
        llcf->access_handlers = handlers;
    }

    if (handlers->nelts >= NGX_HTTP_LUA_MAX_HANDLER_PER_PHASE) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "the number of access_by_lua* directives "
                           "exceeds %d",
                           NGX_HTTP_LUA_MAX_HANDLER_PER_PHASE);
        return NGX_CONF_ERROR;
    }
    ...
    llcf->access_handler = (ngx_http_handler_pt)
                            ngx_http_lua_run_access_handlers;
...
ngx_http_lua_log_by_lua(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    ngx_http_lua_main_conf_t        *lmcf;
    ngx_http_lua_loc_conf_t         *llcf = conf;
    ngx_array_t                     *handlers = llcf->log_handlers;
    ngx_http_lua_phase_handler_t    *ph;
    ...
    if (handlers == NULL) {
        ...
        llcf->log_handlers = handlers;
    }

    if (handlers->nelts >= NGX_HTTP_LUA_MAX_HANDLER_PER_PHASE) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "the number of log_by_lua* directives "
                           "exceeds %d",
                           NGX_HTTP_LUA_MAX_HANDLER_PER_PHASE);
        return NGX_CONF_ERROR;
    }
    ...
    llcf->log_handler = (ngx_http_handler_pt)
                            ngx_http_lua_run_log_handlers;
